...
export class AppComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    try {
      ...
      this.notifications.sendWarning(`There was an issue retrieving latest ffd price.  Ensure your AdBlocker is disabled on this page then reload to see accurate FIAT values.`, { length: 0, identifier: `price-adblock` });
    }
  }
  ...
  performSearch() {
    ...
    const isValidNanoAccount = (
        ( searchData.startsWith('xrb_') || searchData.startsWith('ffd_') )
    ...
    this.notifications.sendWarning(`Invalid ffd address or block hash! Please double check your input`);
  }
  ...
  retryConnection() {
    ...
    this.notifications.sendInfo(`Attempting to reconnect to ffd node`);
  }
